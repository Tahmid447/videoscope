...
function advance(j){j.next_url=j.pending_pages.shift()||null;nextOrderOrGap(j);}
function coverage(j,c){
 ...
 if(j.expected!==null&&c.items.length===j.expected)return 'source count matched';
 ...
 try{
  if(j.stage==='listing'){
   ...
   try{
    ...
    if(!feed)rememberOrders(j,fetched.html,fetched.url);
    ...
    if(Number.isFinite(result.expected)&&(!j.search_root||sameCountScope(fetched.url,j.search_root))){
   ...
   j.pages++;
   const fp=createHash('sha256').update(orderContext(target)+'|'+result.items.map(x=>x.id).sort().join('|')).digest('hex');
   ...
   j.items=c.items.length;advance(j);j.message='Read '+j.pages+' source pages: '+c.items.length+(j.expected!==null?' of '+j.expected+' reported':'')+' videos collected.';
