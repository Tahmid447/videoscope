...
import {parseListing,parseDetail,VERSION} from './extract.mjs';
...
export async function scanStep(job,c,read=readPublic,robots=readRobots){
 ...
 try{
  if(job.stage==='listing'){
   ...
   const fetched=await read(target),result=parseListing(fetched.html,fetched.url,at);job.pages++;job.visited.push(target);c.pages=job.pages;if(job.pages===1){c.label=result.title;job.expected=result.expected;c.expected_count=result.expected}
   ...
   if(!incoming.length&&!result.candidates.length)throw new Error('The returned page contained no verified video entries. Navigation links were rejected rather than shown as videos.');
