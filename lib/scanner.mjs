...
import {parseListing,parseDetail,parseFeed,feedURL,parsePlayerMeta,VERSION} from './extract.mjs';
 ...
 try{
  if(job.stage==='listing'){
   ...
   for(const task of batch){
    ...
    if(current?.players?.length&&(!current.thumbnail_url||current.views===null||current.duration_seconds===null)){
     for(const mirror of current.players.slice(0,2)){
      try{
       const rules=await robots(mirror.url);if(!allowedByRobots(rules,mirror.url))continue;
       const fetched=await read(mirror.url),pm=parsePlayerMeta(fetched.html,fetched.url);
       if(!current.thumbnail_url&&pm.thumbnail_url){current.thumbnail_url=pm.thumbnail_url;current.field_sources={...current.field_sources,thumbnail_url:'public player og:image'}}
       if(current.views===null&&pm.views!==null){current.views=pm.views;current.field_sources={...current.field_sources,views:'public player page text'}}
       if(current.duration_seconds===null&&pm.duration_seconds!==null){current.duration_seconds=pm.duration_seconds;current.field_sources={...current.field_sources,duration_seconds:'public player metadata'}}
       if(current.thumbnail_url&&current.views!==null&&current.duration_seconds!==null)break;
      }catch{}
     }
    }
    job.details_checked++;
