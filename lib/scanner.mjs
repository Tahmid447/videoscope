...
export async function scanStep(job,c,read=readPublic,robots=readRobots){
 ...
 try{
  if(job.stage==='listing'){
   ...
   else try{fetched=await read(target);result=/application\/(?:rss|atom)\+xml|(?:application|text)\/xml/i.test(fetched.contentType||'')?parseFeed(fetched.html,fetched.url,at):parseListing(fetched.html,fetched.url,at)}
