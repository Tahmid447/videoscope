...
export async function scanStep(job,c,read=readPublic,robots=readRobots){
 ...
 try{
  if(job.stage==='listing'){
   ...
   if(/\/feed\/?$/.test(new URL(target).pathname)){fetched=await read(target,{html:false});result=parseFeed(fetched.html,fetched.url,at);usedFeed=true}
   else try{
    fetched=await read(target);result=/application\/(?:rss|atom)\+xml|(?:application|text)\/xml/i.test(fetched.contentType||'')?parseFeed(fetched.html,fetched.url,at):parseListing(fetched.html,fetched.url,at);
    if(!result.items.length){const fallback=feedURL(target);if(allowedByRobots(job.robots,fallback)){try{const feed=await read(fallback,{html:false}),parsed=parseFeed(feed.html,feed.url,at);if(parsed.items.length){fetched=feed;result=parsed;usedFeed=true;note(job,'The normal page exposed no verified video records to the server, so VideoScope used the site’s public RSS/Atom feed instead.')}}catch{}}}
   ...
   for(const task of batch){
    ...
    try{if(!allowedByRobots(job.robots,task.url))throw new Error('Detail page not allowed by source crawl instructions.');const fetched=await read(task.url);const detail=parseDetail(fetched.html,fetched.url,existing,at);if(detail.is_video){if(index>=0)c.items[index]=detail;else c.items.push(detail)}else if(task.candidate)job.rejected_candidates++;else if(index>=0)c.items[index].detail_status='No public detail metadata returned'}catch(e){detailError=e}
    const current=index>=0?c.items[index]:existing;
    if(current&&!current.thumbnail_url){
     try{
      const api=wordpressPostURL(task.url);
      if(api){
       const rr=await robots(api);
       if(allowedByRobots(rr,api)){
        const wp=await read(api,{html:false});
        const meta=parseWordPressPostMeta(wp.html,wp.url);
        if(meta?.thumbnail_url){current.thumbnail_url=meta.thumbnail_url;current.field_sources={...current.field_sources,thumbnail_url:'public WordPress featured image API'};current.detail_status=current.detail_status||'public WordPress metadata read'}
       }
      }
     }catch{}
    }
    if(detailError){job.details_failed++;if(index>=0&&!c.items[index].thumbnail_url)c.items[index].detail_status=detailError.message;note(job,'Some details could not be read; source listing values were preserved.')}
    job.details_checked++;
   }
