...
export const providers: Provider[] = [youtube, tokyomotion, meta, wordpressRss, generic];
...
export function detect(raw: string): Detection {
  ...
  for (const p of [youtube, tokyomotion, meta, generic]) { const source = p.detect(u); if (source) return source; }
